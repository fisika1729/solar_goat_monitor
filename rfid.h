...
static bool rfid_readBytesWithTimeout(Stream &s, uint8_t *buf, size_t len, uint32_t timeoutMs) {
...
static void buzzerBeep(uint16_t onMs) {
  buzzerOn();
  delay(onMs);
  buzzerOff();
}
...
static bool rfid_sendCommand(uint8_t cmd,
  ...
  for (int attempt = 0; attempt < retries; ++attempt) {
    ...
    uint8_t header[4];
    if (!rfid_readBytesWithTimeout(Serial1, header, 4, 50)) {
    ...
    if (!rfid_readBytesWithTimeout(Serial1, respRaw, remaining, 50)) {
    ...
    uint8_t calc = rfid_calcChecksum(chkBuf, chkLen);
    if (calc != rxChk) {
...
// ===================== HIGH-LEVEL RFID COMMANDS =====================

static bool rfid_resetReader() {
  uint8_t resp[32];
  size_t respLen = 0;

  Serial.println(F("[RFID] Sending RESET (0x68)"));

  if (!rfid_sendCommand(UHF_CMD_RESET, nullptr, 0, resp, sizeof(resp), respLen, 3)) {
...
static bool rfid_setPowerMax() {
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    if (!rfid_sendCommand(UHF_CMD_SET_POWER,
...
static bool rfid_stopInventory() {
  uint8_t resp[32];
  size_t respLen;

  if (!rfid_sendCommand(UHF_CMD_STOP_INVENTORY,
                        nullptr, 0,
                        resp, sizeof(resp), respLen, 3)) {
    return false;
  }
...
static bool rfid_setHeartbeatSwitch(bool enable) {
  uint8_t params[7];
  ...
  if (!rfid_sendCommand(UHF_CMD_HEARTBEAT_SWITCH,
...
static bool rfid_startInventory(uint16_t count) {
  uint8_t params[2] = {
  ...
  if (!rfid_sendCommand(UHF_CMD_CONTINUOUS_INVENTORY,
                        params, sizeof(params),
                        resp, sizeof(resp), respLen, 3)) {
    return false;
  }
...
static void rfid_processContinuousInventoryResponse(const uint8_t *resp, size_t respLen) {
  if (respLen < 5) return;
  ...
  rfid_addTagIfNew(tag);
}

static void rfid_readTagsLoop() {
  while (Serial1.available() >= 4) {
    ...
    if (!rfid_readBytesWithTimeout(Serial1, buf, remaining, 20)) {
    ...
    if (cmd == UHF_CMD_CONTINUOUS_INVENTORY_RESP) {
      rfid_processContinuousInventoryResponse(buf, payloadLen);
...
inline void rfid_loop() {
  ...
  // ADC print every 3 seconds, also to BLE
  if (millis() - g_lastAdcPrintMs > 3000) {
    g_lastAdcPrintMs = millis();
    float vin = rfid_readInputVoltage();

    Serial.print(F("[ADC] Vin ≈ "));
    Serial.print(vin, 2);
    Serial.println(F(" V"));

    String msg = "[ADC] Vin ≈ " + String(vin, 2) + " V";
    bleDebugPrintLn(msg);
  }
}
